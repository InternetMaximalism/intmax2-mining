...
contract V1WithdrawalPlonkVerifierV2 {
    ...
    uint256 private constant VK_QL_COM_X =
        6360257186185327540342460023329953297569087091254360572705544656929042421862;
    uint256 private constant VK_QL_COM_Y =
        5907083724563543649059712186166977792502944033451274068745988645662925562896;
    uint256 private constant VK_QR_COM_X =
        19575378299508238408670891404463850272071739890098630552755244543373922680248;
    uint256 private constant VK_QR_COM_Y =
        7351703815096866353650613862000973221747218320877002366442082169606297993317;
    uint256 private constant VK_QM_COM_X =
        1952166171186599363280230887978667470683052388725702762267221486911047408679;
    uint256 private constant VK_QM_COM_Y =
        20102978566317651539033684382797466658387352128524889528609404160375412141956;
    uint256 private constant VK_QO_COM_X =
        21503822883072311311078708175491925118733240534185051394576531798117106806313;
    uint256 private constant VK_QO_COM_Y =
        2261514598444449681308674433099822890933238129476483786628672015180619725718;
    uint256 private constant VK_QK_COM_X =
        4655498409044378111492455204262661848705921273601460969004815648722695597926;
    uint256 private constant VK_QK_COM_Y =
        16367869074223689233273068768780280957786515448191710317887144668560432933437;

    uint256 private constant VK_S1_COM_X =
        8665636640036965962345791216966172795771236641486544092225096278605961707194;
    uint256 private constant VK_S1_COM_Y =
        12561113290574989363810600793898128772007635272350505737150606583386864622164;

    uint256 private constant VK_S2_COM_X =
        12068590867297419351372662337459948612277026717902845163466725513937984505056;
    uint256 private constant VK_S2_COM_Y =
        6117323976350491110898661301557616695113362145968986235207716095920778407327;

    uint256 private constant VK_S3_COM_X =
        11688007033783915184443889924184759139079430692321214834892046380802836102738;
    uint256 private constant VK_S3_COM_Y =
        5583529625591594000461485044325058779384476162692210226063159378228464968889;

    uint256 private constant VK_COSET_SHIFT = 5;

    uint256 private constant VK_QCP_0_X =
        15243488678800871385957301511183787173005780665348013042926515231162261066509;
    uint256 private constant VK_QCP_0_Y =
        3100016535555428663739542634451299519466518323588019889109169122860695751441;
    ...
    function Verify(
        ...
        assembly {
                ...
                -> alpha_not_reduced
            {
                ...
                // Bsb22Commitments
                let proof_bsb_commitments := add(
                    aproof,
                    PROOF_COMMITMENTS_WIRES_CUSTOM_GATES
                )
                ...
                -> pi_commit
            {
                ...
                ith_lagrange := compute_ith_lagrange_at_z(
                    z,
                    zpnmo,
                    add(nb_public_inputs, VK_INDEX_COMMIT_API0),
                    mPtr
                )
                pi_commit := addmod(
                    pi_commit,
            ...
            function fold_state(aproof) {
                ...
                mstore(
                    ...
                    mload(add(state, STATE_FOLDED_H_X))
                )
                mstore(
                    add(state, STATE_FOLDED_DIGESTS_Y),
                    mload(add(state, STATE_FOLDED_H_Y))
                )
                mstore(
            ...
            function compute_commitment_linearised_polynomial_ec(
                ...
                for {
                    ...
                    mstore(
                        add(mPtr, 0x20),
                        calldataload(add(commits_api, 0x20))
                    )
                    point_acc_mul(
                        add(state, STATE_LINEARISED_POLYNOMIAL_X),
                        mPtr,
            ...
            function verify_quotient_poly_eval_at_zeta(aproof) {
                ...
                mstore(
                    s1,
                    addmod(mload(s1), mload(add(state, STATE_GAMMA)), R_MOD)
                )
                mstore(
                    s1,
                    addmod(
                ...
                mstore(
                    s2,
                    addmod(mload(s2), mload(add(state, STATE_GAMMA)), R_MOD)
                )
                mstore(
                    s2,
                    addmod(
                ...
                mstore(
                    s1,
                    mulmod(mload(s1), mload(add(state, STATE_ALPHA)), R_MOD)
                )
                mstore(
                    s1,
                    mulmod(
                ...
                mstore(
                    computed_quotient,
                    addmod(mload(computed_quotient), mload(s1), R_MOD)
                )
                mstore(
                    computed_quotient,
                    addmod(
                ...
                mstore(
                    ...
                    mulmod(
                        calldataload(
                            add(aproof, PROOF_QUOTIENT_POLYNOMIAL_AT_ZETA)
                        ),
                ...
                mstore(
                    ...
                    eq(mload(computed_quotient), mload(s2))
                )
            }
            ...
            function point_acc_mul(dst, src, s, mPtr) {
                ...
                l_success := and(
                    l_success,
                    staticcall(gas(), 6, mPtr, 0x80, dst, 0x40)
                )
                if iszero(l_success) {
                    error_ec_op()
                }
